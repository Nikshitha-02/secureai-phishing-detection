...
const AI_UNAVAILABLE = 'AI explanation is temporarily unavailable.';
...
/** Border + background tint for the AI card based on risk level */
const aiBorder: Record<ScanResult['riskLevel'], string> = {
  safe: 'border-cyan-500/20 bg-cyan-500/5',
  suspicious: 'border-yellow-500/20 bg-yellow-500/5',
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        aria-label="AI Security Analysis"
      >
        {/* Section header */}
        <div className="flex items-center gap-2">
          ...
          <h3 className={[
            ...
            isAiUnavailable ? 'text-gray-500' : 'text-cyan-400',
          ].join(' ')}>
            🤖 AI Security Analysis
          </h3>
        </div>
        ...
        {!isAiUnavailable && (
          <p className="text-xs text-gray-600 pt-1 border-t border-white/5">
            Generated by Google Gemini · Explanation only — the risk score is determined by the rule engine.
          </p>
        )}
      </motion.div>
    </motion.div>
